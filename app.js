...
// ساختار درخت باینری - با فاصله‌گذاری بسیار بهتر
async function buildBinaryTree(rootId) {
    const tree = {};
    const queue = [];
    let processedCount = 0;
    const maxProcess = 200; // کاهش برای جلوگیری از شلوغی
    
    expandedNodes.add(rootId);
    
    const isMobile = window.innerWidth <= 768;
    
    // فاصله‌های بسیار بیشتر
    const baseSpacingMultiplier = isMobile ? 100 : 200;
    const levelHeight = isMobile ? 150 : 250;
    ...
    while (queue.length > 0 && processedCount < maxProcess) {
        ...
        try {
            ...
            if (tree[id].expanded && hasChildren) {
                // فاصله‌گذاری بسیار بیشتر برای سطوح پایین‌تر
                const baseSpacing = Math.pow(2, 7 - Math.min(level, 7)) * baseSpacingMultiplier;
...
// رندر درخت باینری با زوم و فاصله‌گذاری بهتر
function renderBinaryTree(treeStructure) {
    ...
    `;
    treeElement.appendChild(zoomControls);
    ...
    nodesContainer.style.minHeight = '800px';
    nodesContainer.style.padding = '50px';
    ...
    const centerOffsetY = Math.abs(minY) + 100;
    
    // عرض بسیار بیشتر برای جلوگیری از همپوشانی
    const requiredWidth = Math.max(2000, (maxX - minX) + 800);
    ...
    allNodes.forEach(node => {
        if (node.parentId && treeStructure[node.parentId]) {
            ...
            line.setAttribute('y1', parentY + 60);
            line.setAttribute('x2', finalX);
            line.setAttribute('y2', finalY - 60);
