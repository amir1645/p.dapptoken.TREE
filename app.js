...
// ساختار درخت باینری کامل - با فاصله‌گذاری بهتر
async function buildBinaryTree(rootId) {
    const tree = {};
    const queue = [];
    let processedCount = 0;
    const maxProcess = 300; // کاهش برای جلوگیری از همپوشانی
    
    expandedNodes.add(rootId);
    
    const isMobile = window.innerWidth <= 768;
    
    // فاصله‌های بیشتر برای جلوگیری از همپوشانی
    const baseSpacingMultiplier = isMobile ? 60 : 120; // افزایش فاصله
    const levelHeight = isMobile ? 120 : 180; // افزایش فاصله عمودی
    ...
    while (queue.length > 0 && processedCount < maxProcess) {
        ...
        try {
            ...
            if (tree[id].expanded && hasChildren) {
                ...
                const baseSpacing = Math.pow(2, 6 - Math.min(level, 6)) * baseSpacingMultiplier;
                ...
                if (hasLeft) {
                    ...
                    // بررسی عدم همپوشانی
                    let collision = false;
                    Object.values(tree).forEach(existingNode => {
                        if (Math.abs(existingNode.x - leftX) < 100 && Math.abs(existingNode.y - leftY) < 80) {
                            collision = true;
                        }
                    });
                ...
                if (hasRight) {
                    ...
                    let collision = false;
                    Object.values(tree).forEach(existingNode => {
                        if (Math.abs(existingNode.x - rightX) < 100 && Math.abs(existingNode.y - rightY) < 80) {
                            collision = true;
                        }
                    });
...
function renderBinaryTree(treeStructure) {
    ...
    nodesContainer.style.minHeight = '600px';
    nodesContainer.style.padding = '20px';
    ...
    // محاسبه عرض مورد نیاز با فاصله بیشتر
    const requiredWidth = Math.max(1200, (maxX - minX) + 400);
    ...
    allNodes.forEach(node => {
        if (node.parentId && treeStructure[node.parentId]) {
            ...
            line.setAttribute('y1', parentY + 45);
            line.setAttribute('x2', finalX);
            line.setAttribute('y2', finalY - 45);
...
// ایجاد المان نود باینری - بهبود یافته
function createBinaryNodeElement(node, x, y) {
    ...
    nodeDiv.innerHTML = `
        ...
        <div class="node-info">
            ...
            <div class="info-line">
                <span class="info-label ${node.position === 'left' ? 'highlight-left' : ''}">چپ:</span>
                <span class="info-value ${node.position === 'left' ? 'highlight-left' : ''}">${leftText}</span>
            </div>
            <div class="info-line">
                <span class="info-label ${node.position === 'right' ? 'highlight-right' : ''}">راست:</span>
                <span class="info-value ${node.position === 'right' ? 'highlight-right' : ''}">${rightText}</span>
            </div>
        </div>
        ${childrenSummary}
    `;
